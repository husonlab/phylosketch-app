 ...
 */
public class SetFontSizeCommand extends UndoableRedoableCommand {
	 ...
	 * constructor
	 */
	public SetFontSizeCommand(Document document, double size) {
		super("font size");
		...
		if (nodeData.size() > 0 || edgeData.size() > 0) {
			undo = () -> {
				nodeData.forEach(id -> {
					...
					var label = networkView.getView(v).label();
					label.setFontSize(oldNodeFontSize.get(id));
					document.getModel().getTree().setLabel(v, label.getText());
				});
				edgeData.forEach(id -> {
					var e = tree.findEdgeById(id);
					var label = networkView.getView(e).label();
					label.setFontSize(oldEdgeFontSize.get(id));
