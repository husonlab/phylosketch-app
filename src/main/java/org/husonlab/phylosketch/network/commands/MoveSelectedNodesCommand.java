...
public class MoveSelectedNodesCommand extends UndoableRedoableCommand {
	...
	public MoveSelectedNodesCommand(double dx, double dy, NetworkView networkView, ObservableSet<Node> selectedItems,
		...
		undo = () -> {
			...
			//edgeData.forEach(id -> networkView.getEdge2view().get(tree.findEdgeById(id)).setControlCoordinates(oldEdgeControlCoordinates.get(id)));
		};

		redo = () -> {
			nodeData.forEach(id -> networkView.moveNode(tree.findNodeById(id), dx, dy));

			//edgeData.forEach(id -> networkView.getEdge2view().get(tree.findEdgeById(id)).setControlCoordinates(newEdgeControlCoordinates.get(id)));
		};
	}
