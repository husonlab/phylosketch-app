/*
 * EmbeddingOptimizer.java Copyright (C) 2022 Daniel H. Huson
 *
			...
			if (trees.length == 2 && (taxaTrees[0].size() != taxon2ID.size() || taxaTrees[1].size() != taxon2ID.size())) {
				...
				for (var s = 0; s < 2; s++) {
					...
					for (var it = taxaTrees[s].iterator(); it.hasNext(); ) {
						var taxon = it.next();
						var contains = (taxaTrees[Math.abs(s - 1)]).contains(taxon);
						if (!contains) {
				...
				for (var s = 0; s < 2; s++) {
					for (Iterator<String> it = taxaNotInTrees[Math.abs(s - 1)].iterator(); it.hasNext(); ) {
